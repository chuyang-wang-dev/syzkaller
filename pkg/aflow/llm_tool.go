...
type StructuredLLMTool[State, Args, Results any] struct {
	...
	// Native structured validation hook (this intercepts the 'set-results' tool call).
	// It is designed to return (Results, error), allowing the logic to canonicalize,
	// overwrite, or compute derivative fields (like diffs) before accepting the output.
	ValidatedOutputs func(ctx *Context, state State, args Args, results Results) (Results, error)
	agent            *LLMAgent
}
...
func (t *StructuredLLMTool[State, Args, Results]) verify(ctx *verifyContext) {
	...
	if t.ValidatedOutputs != nil {
		t.agent.Outputs = ValidatedLLMOutputs[Results, State](func(ctx *Context, state State, res Results) (Results, error) {
			a, _ := ctx.state[llmToolArgs].(Args)
			return t.ValidatedOutputs(ctx, state, a, res)
		})
	} else {
