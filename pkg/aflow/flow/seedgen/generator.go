...
var GeneratorAgent = &aflow.LLMAgent{
	...
	Outputs: aflow.ValidatedLLMOutputs(
		func(ctx *aflow.Context, state struct{}, outputs GeneratorOutputs) (GeneratorOutputs, error) {
			if !outputs.GeneratorGiveUp {
				...
				_, err := crash.LoadSeedProgramDetails(ctx, outputs.ExecutionCachedID)
				if err != nil {
	...
	Instruction: "You are the Generator orchestrating the generation of a syzkaller seed.\n" +
		...
		"These test seeds provide examples for environment, file system, or device setup.\n" +
		...
		"the 'seedgen-analyzer'. You must copy their setup lines directly into your program, " +
		"preserving any \"$BLOB_*\" placeholders exactly as-is.\n\n" +
		"Workflow:\n" +
